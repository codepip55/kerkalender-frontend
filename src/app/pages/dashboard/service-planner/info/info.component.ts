...
export class InfoComponent implements OnInit {
  ...
  async ngOnInit() {
    this.serviceForm = new FormGroup({
      ...
      location: new FormControl(
        'Reggestroom (Reggestraat 100, Enter)',
        Validators.required,
      ),
  ...
  private confirmLeave() {
    if (this.serviceForm.dirty) {
      return confirm(
        'Weet je zeker dat je de pagina wilt verlaten? Je hebt nog niet opgeslagen.',
      );
    }
    return true;
  }
  ...
  private async getService(id: string): Promise<Service> {
    ...
    // @ts-expect-error - service.service is not defined
    return service;
  }
  get teams() {
  ...
  addPosition(index: number, positionName: string) {
    ...
    for (let i = 0; i < positions.length; i++) {
      if (positions.at(i).value === positionName) {
        this.alertService.add({
          type: 'warning',
          message: 'Deze positie bestaat al in dit team.',
        });
        return;
      }
    }
        ...
        this.apiService
          ...
          .subscribe({
            next: data => {
              this.alertService.add({
                type: 'success',
                message: 'Service is aangemaakt.',
              });
              // @ts-expect-error - data._id is not defined
  ...
  deleteService() {
    if (this.id === 'new') {
      ...
      this.alertService.add({
        type: 'warning',
        message: 'Service is niet aangemaakt.',
      });
    }
